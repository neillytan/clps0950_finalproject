function response_cell = Fex_Code(original)
%notes for grading:

% All DrawFormattedText and Screen commands were written by Jenny (the
% current display is entirely her work)

% Salomi created the original logic psuedocode, incorporated the comments functionality, 
% and wrote the section that writes a csv file

% Maya wrote most of the functional logic code, initializing variables,
...
for ii= 1:size(original,2)
    for jj= 1:size(original,1)
        ...
        %Salomi edited this base code to make it work for our program
        Screen('FrameRect',window, black, [300 300 700 500],4);
        msg = ['Your comments: '];
        DrawFormattedText(window,msg,320,320,0,[],0,0,vLineSpacing);
